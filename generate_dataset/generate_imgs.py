...
if __name__ == '__main__':
    ...
    for iter in range(1,1000):
        ...
        total_points_sampled = 10
        step_size_1 = len(interpolated_path)//total_points_sampled
        step_size_2 = len(interpolated_park_path)//total_points_sampled
        # sampled_points = final_path[::step_size][:10] ##First 50 points at the spacong of 10

        interpolate_path_sampled = interpolated_path[::step_size_1][:total_points_sampled]
        interpolated_park_path_sampled = interpolated_park_path[::step_size_2][:total_points_sampled]


        f = open(f"generate_dataset/output_points/environment{iter}.txt","w")
        f.write(np.array2string(interpolate_path_sampled)[1:-1]+"\n"+np.array2string(interpolated_park_path_sampled)[1:-1])
        ...
        env_path.draw_path(interpolate_path_sampled)
        env_path.draw_path(interpolated_park_path_sampled)
