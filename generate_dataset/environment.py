...
class Environment:
    ...
    def draw_path(self, path):
        ...
        for p in path:
            self.background[p[1]+10*self.margin:p[1]+10*self.margin+6,p[0]+10*self.margin:p[0]+10*self.margin+6]=color


    def draw_goal(self, path):
        path = np.array(path)*10
        color = np.array([0.1,0.1,0.1])
        path = path.astype(int)
       
        p = path[-1]
        self.background = cv2.circle(self.background, (p[0]+10*self.margin,p[1]+10*self.margin), 20, [255/255, 0/255, 0/255], -1)
